...
def main():
    ...
    rep.p("Что проверяется. Главное — распознавание ФИО: это единственное, что делает нейросетевая модель; "
          "остальные типы ПДн ищут правила. Утечки остальных ПДн — в разделе 9.")
    ...
    card("2.9. Утечки остальных ПДн", [
        ("Что проверяет", "сколько телефонов, паспортов, адресов и других ПДн осталось в тексте после обезличивания"),
        ("Данные", "hivetrace — все типы, которые у нас есть"),
        ("Как проводили", "значение считается утёкшим, если оно целиком осталось в выходном тексте; последний прогон 18.09"),
        ("Что показал", "см. раздел 9: основная часть утечек — адреса"),
        ("Ограничения", "конкуренты здесь не участвуют (мы запускали их только на ФИО); мера мягкая для адресов — "
                        "частично закрытый адрес утечкой не считается"),
    ])
    # ── Как читать
    ...
    # ── Опубликованные
    rep.h("7. Сравнение с системами из статьи (hivetrace, domain-часть)")
    ...
    # ── Утечки по типам
    rep.h("9. Утечки остальных ПДн (hivetrace, последний прогон)")
