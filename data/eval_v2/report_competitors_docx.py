...
def main():
    ...
    ], "Для «ложное ФИО» лучше меньшее значение, для остальных метрик — большее. В ячейке конкурента: "
       "его значение, разница «мы минус он» и её интервал.")
    ...
    for name, v in published:
        ...
        rows.append([name + " (статья)", (f"{fmt('f1', v)} → разница {fmt_diff('f1', ours - v)}; {WORD[verdict]}",
                                          verdict)])
    for s in ("S6", "S7"):
        res = dom["pairs"][f"S5-v3|{s}"]["f1"]
        rows.append([SYS_RU[s] + " (наш замер)", (f"{fmt('f1', dom['systems'][s]['f1'])} → разница "
                                                  f"{fmt_diff('f1', res['diff'])}, ДИ {fmt_ci('f1', res['ci95'])}; "
                                                  f"{WORD[res['verdict']]}", res["verdict"])])
    rep.table(["Система", f"F1 по ФИО (наш: {fmt('f1', ours)})"], rows,
    ...
    for ds, name in (("hive", "hivetrace"), ("alro", "alrosait"), ("mcr", "MultiCoNER")):
        rows.append([name, "3,5 (модель, замер на test_v2)",
                     (f"{L[ds]['S6']:.1f}".replace(".", ","), speed(3.47, L[ds]["S6"])),
                     (f"{L[ds]['S7']:.1f}".replace(".", ","), speed(3.47, L[ds]["S7"])),
                     (f"{L[ds]['S5-v3']:.1f} (весь сервис)".replace(".", ","), "na")])
    rep.table(["Набор", "Наша модель", SYS_RU["S6"], SYS_RU["S7"], "Наш сервис целиком"], rows)
    ...
    for ds, name in (("v2", "test_v2"), ("bench", "pii_benchmark"), ("ext", "factRuEval")):
        ...
        for k, lower, m in keys:
            ...
            txt = (f"{before * 100:.1f} → {after * 100:.1f} %" if pct else f"{before:.3f} → {after:.3f}").replace(".", ",")
            row.append((txt + f"\n{WORD[verdict]}", verdict))
        rows.append(row)
    rep.table(["Набор", "F1 по ФИО", "Имён скрыто", "Ложное ФИО", "Утечки ПДн"], rows,
