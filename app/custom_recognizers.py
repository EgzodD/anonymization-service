...
_ADDR_HOUSE = (rf"(?:\s*,?\s*(?:(?:д\.|дом|влд\.|владени[ея]|стр\.|строени[ея]|корп\.?|корпус[ае]?|к\.)"
...
_ADDR_PATTERNS = [
    ...
    Pattern(name="address_reverse", score=0.95, regex=(
        rf"{_ADDR_INDEX}(?:{_ADDR_CITY}\s*,\s*)*{_CAP}{_LOW}+(?:ый|ий|ой|ая|ое|ский|ская|ское)\s+"
    ...
    Pattern(name="address_bare", score=0.95, regex=(
        rf"{_ADDR_INDEX}(?:{_ADDR_CITY}\s*,\s*)?(?<![\w\-]){_ADDR_BARE_STREET}\s+{_ADDR_HOUSENUM}"
